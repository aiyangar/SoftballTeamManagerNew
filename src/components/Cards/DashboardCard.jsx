...
const DashboardCard = ({ cardData }) => {
    ...
    return (
        <Link to={linkTo} className="block">
            <div className="bg-neutral-900 shadow rounded-lg pt-4 pb-6 px-6 hover:bg-neutral-800 transition-all duration-300 cursor-pointer min-h-64 flex flex-col dashboard-card border border-gray-700 hover:border-gray-500">
                <div className="flex items-center justify-between mb-3">
                    <h3 className="text-lg font-semibold text-white">{title}</h3>
                    <div className={`text-4xl ${iconColor} flex items-center justify-center w-16 h-16`}>
                        {icon}
                    </div>
                </div>
