...
const PaymentForm = ({ gameId, teamId, onClose, onPaymentComplete }) => {
  ...
  const cleanZeroPayments = async () => {
    ...
    const paymentsToDelete = data.filter(payment => {
      const totalPayment =
        (payment.monto_umpire || 0) + (payment.monto_inscripcion || 0);
    ...
    if (paymentsToDelete.length > 0) {
      for (const payment of paymentsToDelete) {
        ...
        if (deleteError) {
          console.error(
            `Error al eliminar pago en 0 para jugador ${payment.jugador_id}:`,
            deleteError
          );
        }
      }
  ...
  const handleSubmit = async e => {
    ...
    const totalPayment =
      (montoUmpire ? parseFloat(montoUmpire) : 0) +
      (montoRegistro ? parseFloat(montoRegistro) : 0);
    ...
    if (
      existingPayments[selectedPlayer] &&
      !showUpdateWarning &&
      !showCancelWarning
    ) {
    ...
    const umpireFaltante =
      paymentTotals.umpireTarget - paymentTotals.totalUmpire;
    ...
    if (existingPayments[selectedPlayer]) {
      ...
      // Insert new payment
      result = await supabase.from('pagos').insert([paymentData]);
    }
          ...
          <form id='payment-form' onSubmit={handleSubmit} className='space-y-4'>
              ...
              {players.length === 0 ? (
                <div className='text-yellow-500 p-3 bg-gray-800 rounded-md border border-gray-600'>
                  ...
                  <span className='text-sm'>
                    Primero debes marcar la asistencia de los jugadores.
                  </span>
                </div>
              ) : (
                <select
            ...
            <div className='bg-blue-900 border border-blue-600 text-blue-200 px-3 py-2 rounded text-xs'>
              <div className='flex items-center space-x-2'>
                ...
                <span>
                  Si el monto del umpire excede lo necesario, el exceso se
                  transferirá automáticamente a inscripción.
                </span>
              </div>
            </div>
            ...
            {showUpdateWarning && (
              <div className='bg-yellow-900 border border-yellow-600 rounded-md p-3'>
                ...
                <div className='text-yellow-100 text-sm mb-3'>
                  Este jugador ya tiene un pago registrado para este partido. Al
                  continuar, se actualizará el pago existente.
                </div>
            ...
            {showCancelWarning && (
              <div className='bg-red-900 border border-red-600 rounded-md p-3'>
                ...
                <div className='text-red-100 text-sm mb-3'>
                  Estás a punto de <strong>borrar completamente</strong> el pago
                  de este jugador. El registro será eliminado de la base de
                  datos y el jugador aparecerá como no pagado.
                  <br />
                  <br />
