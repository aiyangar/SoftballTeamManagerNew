...
const Dashboard = () => {
  ...
  const fetchTeamInfo = async teamId => {
    try {
      ...
      const nextGame =
        nextGameData && nextGameData.length > 0 ? nextGameData[0] : null;

      // Obtener último partido jugado
      const { data: lastGameData, error: lastGameError } = await supabase
        .from('partidos')
        .select(
          'id, equipo_contrario, fecha_partido, lugar, carreras_equipo_local, carreras_equipo_contrario, resultado, finalizado'
        )
        .eq('equipo_id', teamId)
        .eq('finalizado', true)
      ...
      const lastGame =
        lastGameData && lastGameData.length > 0 ? lastGameData[0] : null;
      ...
      const gameStats = {
        wins:
          allGames?.filter(game => game.resultado === 'Victoria').length || 0,
        losses:
          allGames?.filter(game => game.resultado === 'Derrota').length || 0,
      ...
      const teamData =
        teamDataArray && teamDataArray.length > 0 ? teamDataArray[0] : null;
      if (!teamData) {
      ...
      const { data: payments, error: registrationPaymentsError } =
        await supabase
          .from('pagos')
      ...
      const totalRegistrationPaid = payments.reduce(
        (sum, payment) => sum + (payment.monto_inscripcion || 0),
        0
      );
      const totalRegistrationRequired = teamData.inscripcion || 0;
      const remainingRegistration = Math.max(
        0,
        totalRegistrationRequired - totalRegistrationPaid
      );
      ...
      const averageAttendance =
        totalGames > 0
          ? Math.round((attendanceData?.length || 0) / totalGames)
          : 0;
              ...
              // Card: Próximo Juego
              {
                ...
                content: teamInfo.nextGame ? (
                  ...
                  <p className='text-gray-400'>
                    No hay próximos juegos programados
                  </p>
                ),
              },
                ...
                content: (
                  <div className='space-y-2 w-full'>
                    ...
                    <div className='flex justify-between items-center p-2 bg-gray-800 rounded w-full'>
                      ...
                      <span className='text-green-400 font-bold'>
                        {teamInfo.totalGames > 0
                          ? Math.round(
                              (teamInfo.gameStats.wins / teamInfo.totalGames) *
                                100
                            )
                          : 0}
                        %
        ...
        {!selectedTeam && !loadingTeams && (
          <div className='bg-neutral-900 shadow rounded-lg p-8 text-center border border-gray-700'>
            {teams.length === 0 ? (
              ...
              <div>
                <p className='text-gray-400 mb-4'>
                  Tienes múltiples equipos. Selecciona uno para trabajar:
                </p>
